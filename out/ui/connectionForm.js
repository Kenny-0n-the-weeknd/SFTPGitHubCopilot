...
exports.showConnectionForm = showConnectionForm;
 ...
 */
async function showConnectionForm(context, existing) {
    return new Promise((resolve) => {
        ...
        const panel = vscode.window.createWebviewPanel('remoteExplorer.connectionForm', existing ? `Edit: ${existing.label}` : 'Add Remote Connection', vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        panel.webview.onDidReceiveMessage(async (message) => {
            (0, utils_1.logInfo)(`Webview sent message: ${message.command}`);
            switch (message.command) {
                ...
                case 'save': {
                    ...
                    const config = {
                        id: existing?.id || (0, utils_1.generateId)(),
...
function getHtml(webview, context, defaults, isEdit) {
